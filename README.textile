...
bc. okcmd -u your_username -p your_password -f message_output_file.mbox [-m mbox_format]
...
h3. Mailbox formats, formerly known as "Thunderbird" mode

The original plaintext mode has been deprecated; @okcmd@ now saves messages in classic 'mbox' format which is also easy to read in a text editor and doesn't required hand-crafted message formatting. Any of the formats supported by @mailbox@ can be specified with the @-m@ option. The generated mailbox has been tested with alpine and will likely work with Thunderbird if you place it in a @Local@ folder. Please report back if there are any issues!
